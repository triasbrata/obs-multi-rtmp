...
bool obs_module_load()
{
	...
	blog(LOG_INFO,
	     TAG
	     "version: %s by SoraYuki https://github.com/sorayuki/obs-multi-rtmp/",
	     PLUGIN_VERSION);
...
void add_new_target_cb(obs_data_t *request_data, obs_data_t *response_data,
		       void *priv_data)
{
...
{
	auto vendor = obs_websocket_register_vendor("obs-multi-rtmp");
	if (!vendor) {
		blog(LOG_ERROR,
	...
	if (!obs_websocket_vendor_register_request(vendor, "add_new_target",
						   add_new_target_cb, NULL))
		blog(LOG_ERROR,
