...
template<class T> QString to_qstring(T x)
{
...
public:
		...
		: QWidget(parent)
	{
		...
		layout->addWidget(edit_ = new QLineEditWithFocus(updater, this),
				  1);
...
struct PropertyWidget {
	...
	PropertyWidget(QWidget *parent, UpdateHandler *updater, obs_property *p)
	{
		...
		case OBS_PROPERTY_TEXT: {
			auto le = new QLineEditWithEye(
				updater, parent,
				obs_property_text_type(p) == OBS_TEXT_PASSWORD);
			ctrl = le;
			break;
		}
		...
		QObject::connect(label, &QObject::destroyed,
				 [this]() { label = nullptr; });
		QObject::connect(ctrl, &QObject::destroyed,
				 [this]() { ctrl = nullptr; });
		...
		if (obs_property_get_type(p) == propType) {
			...
			case OBS_PROPERTY_LIST: {
				...
				for (auto i = 0; i < cnt; ++i) {
					auto itemname =
						obs_property_list_item_name(p,
									    i);
					QVariant data;
					if (cbType ==
					    obs_combo_format::OBS_COMBO_FORMAT_INT)
						data = obs_property_list_item_int(
							p, i);
					else if (cbType ==
						 obs_combo_format::
							 OBS_COMBO_FORMAT_FLOAT)
						data = obs_property_list_item_float(
							p, i);
					else if (cbType ==
						 obs_combo_format::
						...
						data = LoadCString(
							obs_property_list_item_string(
								p, i));
					cb->addItem(LoadCString(itemname),
						    data);
				}
			}
			}
	...
	void LoadData(obs_data *data)
	{
		...
		case OBS_PROPERTY_INT: {
			...
			static_cast<QLineEditWithEye *>(ctrl)->edit()->setText(
				to_qstring(val));
			break;
		}
		case OBS_PROPERTY_FLOAT: {
	...
	void SaveData(obs_data *data)
	{
		...
		case OBS_PROPERTY_INT: {
			try {
				auto val = tostdu8(
					static_cast<QLineEditWithEye *>(ctrl)
						->edit()
						->text());
				obs_data_set_int(data, name.c_str(),
						 std::stoi(val));
			} catch (...) {
			}
			break;
		...
		case OBS_PROPERTY_TEXT: {
			auto val = static_cast<QLineEditWithEye *>(ctrl)
					   ->edit()
					   ->text();
			obs_data_set_string(data, name.c_str(),
					    tostdu8(val).c_str());
			break;
		}
		case OBS_PROPERTY_LIST: {
...
class QPropertiesWidgetImpl : virtual public QWidget,
			      public QPropertiesWidget,
			      public UpdateHandler {
	std::unordered_map<std::string, std::shared_ptr<PropertyWidget>>
		propwids;
...
public:
	QPropertiesWidgetImpl(obs_properties *props, obs_data *p_settings,
			      QWidget *parent)
		: QWidget(parent),
		  props(props),
	...
	void LoadProperties()
	{
		...
		do {
			...
			if (it == oldpropwids.end()) {
				auto newwid = std::make_shared<PropertyWidget>(
					this, this, x);
				newwid->LoadData(settings);
				propwids.insert(
					std::make_pair(newwid->name, newwid));
	...
	void Save() { obs_data_apply(orig_settings, settings); }
};
}; // namespace

QPropertiesWidget *createPropertyWidget(obs_properties *props,
					obs_data *settings, QWidget *parent)
{
