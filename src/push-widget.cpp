...
public:
	...
	{
		auto thiz = static_cast<IOBSOutputEventHanlder *>(x);
		long long int code = calldata_int(param, "code");
		if (code >= std::numeric_limits<int>::min() &&
		    code <= std::numeric_limits<int>::max()) {
			thiz->OnStopped(static_cast<int>(code));
		}
	}
...
class PushWidgetImpl : public PushWidget, public IOBSOutputEventHanlder {
	...
	bool PrepareOutputService()
	{
		if (!output_) {
			blog(LOG_ERROR, TAG
			     "Prepare output service before output object is created.");
			return false;
		}

		ReleaseOutputService();

		auto conf = obs_data_create_from_json(
			config_->serviceParam.dump().c_str());
		...
		auto service = obs_service_create(
			service_id, "multi-output-service", conf, nullptr);
	...
	bool PrepareEncoderSource()
	{
		if (!output_) {
			blog(LOG_ERROR, TAG
			     "Prepare output scene before output object is created.");
			return false;
		}
		...
		if (!using_main_video_encoder_) {
			...
			if (!venc) {
				blog(LOG_ERROR, TAG
				     "Prepare output scene before encoder is created.");
				return false;
			}
			auto videoConfig =
				FindById(GlobalMultiOutputConfig().videoConfig,
					 config_->videoConfig.value_or(""));

			if (!videoConfig ||
			    !videoConfig->outputScene.has_value()) {
	...
	bool ReleaseOutputSceneView()
	{
		...
		OBSSourceAutoRelease source =
			obs_view_get_source(scene_view_, 0);
		if (source) {
	...
	bool PrepareOutputEncoders()
	{
		if (!output_) {
			blog(LOG_ERROR, TAG
			     "Prepare output encoder before output object is created.");
			return false;
		}
		...
		if (config_->videoConfig.has_value()) {
			...
			venc = obs_get_encoder_by_name(
				VideoEncoderName().c_str());
			if (venc) {
				obs_encoder_release(venc);
			} else {
				auto videoConfigId = *config_->videoConfig;
				auto videoConfig = FindById(global.videoConfig,
							    videoConfigId);
				if (videoConfig) {
					OBSData settings =
					...
					if (venc) {
						...
						obs_encoder_set_frame_rate_divisor(
							...
							videoConfig
								->fpsDenumerator);
						using_main_video_encoder_ =
							false;
					}
				} else {
					blog(LOG_ERROR,
					     TAG
					     "Load video encoder config failed for %s. Sharing with main output.",
					     config_->name.c_str());
				}
			}
		} else {
		...
		if (config_->audioConfig.has_value()) {
			...
			aenc = obs_get_encoder_by_name(
				AudioEncoderName().c_str());
			if (aenc)
				obs_encoder_release(aenc);
			else {
				auto audioConfigId = *config_->audioConfig;
				auto audioConfig = FindById(global.audioConfig,
							    audioConfigId);
				if (audioConfig) {
					OBSData settings =
						obs_data_create_from_json(
					...
					aenc = obs_audio_encoder_create(
						audioConfig->encoderId.c_str(),
						AudioEncoderName().c_str(),
						settings, audioConfig->mixerId,
						nullptr);
					obs_encoder_release(aenc);
					...
					blog(LOG_ERROR,
					     TAG
					     "Load audio encoder config failed for %s. Sharing with main output.",
					     config_->name.c_str());
				}
			}
		} else {
		...
		obs_output_set_audio_encoder(output_, obs_encoder_get_ref(aenc),
					     0);
	...
	bool ReleaseOutputEncoder()
	{
		if (!output_)
			...
			blog(LOG_ERROR,
			     TAG "Release output while it is active.");
			return false;
		}
	}
	...
	void UpdateStreamStatus()
	{
		...
		static const char *units[] = {"bps",  "Kbps", "Mbps",
					      "Gbps", "Tbps", "Pbps",
					      "Ebps", "Zbps", "Ybps"};
		...
		if (interval > 0) {
			...
			sprintf(strDuration, "%02d:%02d:%02d", (int)hh.count(),
				(int)mm.count(), (int)ss.count());

			char strFps[32] = {0};
			sprintf(strFps, "%d FPS",
				static_cast<int>(std::round(
					static_cast<double>(new_frames -
							    total_frames_) /
					static_cast<double>(interval))));
			auto bps =
				(static_cast<double>(new_bytes - total_bytes_) *
				 8.0) /
				static_cast<double>(interval);
			...
			msg_->setText((std::string(strDuration) + "  " +
				       strBps + "  " + strFps)
					      .c_str());
		}
...
public:
	...
	void StartStreaming() override
	{
		...
		if (output_ == nullptr) {
			obs_data *output_settings = obs_data_create_from_json(
				config_->outputParam.dump().c_str());

			auto protocolInfo = GetProtocolInfos()->GetInfo(
				config_->protocol.c_str());
			assert(protocolInfo);
			if (!protocolInfo) {
				blog(LOG_ERROR,
			...
			output_ = obs_output_create(output_id, "multi-output",
		...
		if (output_) {
			...
			if (profileConfig) {
				bool useDelay = config_get_bool(
					profileConfig, "Output", "DelayEnable");
				bool preserveDelay =
					config_get_bool(profileConfig, "Output",
							"DelayPreserve");
				int delaySec = static_cast<int>(config_get_int(
					profileConfig, "Output", "DelaySec"));

				obs_output_set_delay(
					output_, useDelay ? delaySec : 0,
					preserveDelay
	...
	void LoadConfig() { name_->setText(QString::fromUtf8(config_->name)); }
	...
	{
		std::unique_ptr<EditOutputWidget> dlg{createEditOutputWidget(
			targetid_,
