...
static std::string SaveMultiOutputConfig(MultiOutputConfig &config)
{
	...
	for (auto &video_config : config.videoConfig) {
		if (videoconfig_in_use.find(video_config->id) !=
		    videoconfig_in_use.end())
	...
	for (auto &audio_config : config.audioConfig) {
		if (audioconfig_in_use.find(audio_config->id) !=
		    audioconfig_in_use.end())
	...
	blog(LOG_INFO,
	     TAG "Save %d targets, %d video configs, %d audio configs",
	     target_count, videocfg_count, audiocfg_count);
...
static OutputTargetConfigPtr LoadTargetConfig(nlohmann::json &json)
{
	...
	config->protocol = GetJsonField<std::string>(json, "protocol")
				   .value_or("RTMP"); // for compatibility
	config->syncStart =
		GetJsonField<bool>(json, "sync-start").value_or(false);
	config->syncStop = GetJsonField<bool>(json, "sync-stop")
				   .value_or(config->syncStart);
	config->serviceParam =
		GetJsonField<nlohmann::json>(json, "service-param")
			.value_or(nlohmann::json{});
	config->outputParam = GetJsonField<nlohmann::json>(json, "output-param")
				      .value_or(nlohmann::json{});
...
static VideoEncoderConfigPtr LoadVideoConfig(nlohmann::json &json)
{
	...
	config->id = *id;
	config->encoderId =
		GetJsonField<std::string>(json, "encoder").value_or("");
	...
	config->fpsDenumerator =
		GetJsonField<int>(json, "fps-denumerator").value_or(1);
	config->encoderParams = GetJsonField<nlohmann::json>(json, "param")
					.value_or(nlohmann::json{});
...
static MultiOutputConfig LoadMultiOutputConfig(const std::string &content)
{
	try {
		...
		if (it != json.end() &&
		    it->type() == nlohmann::json::value_t::array) {
		...
		if (it != json.end() &&
			...
			for (auto &video_enc_json : *it) {
				if (video_enc_json.type() !=
				    nlohmann::json::value_t::object)
					continue;
				auto video_enc =
					LoadVideoConfig(video_enc_json);
				if (video_enc) {
					config.videoConfig.emplace_back(
						video_enc);
				}
				++videocfg_count;
			}
		...
		if (it != json.end() &&
			...
			for (auto &audio_enc_json : *it) {
				if (audio_enc_json.type() !=
				    nlohmann::json::value_t::object)
					continue;
				auto audio_enc =
					LoadAudioConfig(audio_enc_json);
				if (audio_enc) {
					config.audioConfig.emplace_back(
						audio_enc);
				}
				++audiocfg_count;
			}
		...
		blog(LOG_INFO,
		     TAG "Load %d targets, %d video configs, %d audio configs",
		     target_count, videocfg_count, audiocfg_count);
...
void SaveMultiOutputConfig()
{
	...
	if (profiledir) {
		...
		os_quick_write_utf8_file_safe(filename.c_str(), content.c_str(),
					      content.size(), true, "tmp",
					      "bak");
	...
	if (profiledir) {
		...
		if (content) {
			GlobalMultiOutputConfig() =
				LoadMultiOutputConfig(content);
			bfree(content);
			ret = true;
			blog(LOG_INFO, TAG "Load config from %s",
			     filename.c_str());
		} else {
			blog(LOG_INFO, TAG "Load config from %s failed",
			     filename.c_str());
		}
	}
	bfree(profiledir);
...
template<class T> static bool has_id(T &container, const std::string &id)
{
